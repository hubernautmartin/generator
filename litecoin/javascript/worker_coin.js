...
function worker_coin_generate_outputs(private_key_string, coin_split_key_boolean) {
  ...
  var derived_path = root.derivePath("m/44'/2'/0'/0/0"); // 2 = Litecoin, https://github.com/satoshilabs/slips/blob/master/slip-0044.md
  var formatted_private_key_string = window.wif.encode(176, new Buffer(derived_path.privateKey, 'hex'), true) // 176 = 0xb0
  // https://github.com/libbitcoin/libbitcoin/wiki/Altcoin-Version-Mappings
  var address_string = window.bitcoin.payments.p2pkh({ pubkey: derived_path.publicKey, network: { pubKeyHash: 0x30 }}).address;
